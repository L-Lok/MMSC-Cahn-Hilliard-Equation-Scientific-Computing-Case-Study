...
def coeff_matrix(h, dt, N, c, tsp, eps):
    ...
    diag_part3 = []
    for i in range(N + 1):
        diag_part3.append(- 1 / eps * (c[tsp][i]) ** 2 - 2 * eps / h**2)
...
def simulation(lb, ub, N, n, dt = None, power=4, eps=0.01):
    ...
    np.savetxt('SD 1D.txt', c)
    return c, dt

def simD(lb, ub, N, n = None, dt = None, T_final = None, power=4, eps=0.01, error = 1e-3, i = None):
    """
    lb:     lower bound
    ub:     upper bound
    N + 1:  the number of grid points
...
def plotting(lb, ub, N, n, dt, power):
    ...
    ax.plot(x, c[18], label= f"t = {18*dt:.6f}")
    ax.legend(loc="best", fontsize = 12)
    ax.grid()
    ax.set_xlabel("x", fontsize = 20)
    ax.set_ylabel("c(x, t)", fontsize = 20)
    ax.set_title("Cahn-Hilliard Equation Scheme D", fontsize = 20)
    ...
    plt.show()
    fig.savefig(f"./Semi Implicit Scheme D Plot dt = h^{power}")
...
if __name__ == "__main__":
    ...
    n = 3227
    power = 2
    c, dt = simulation(lb=lb, ub=ub, dt= None,N=N, n=n, power=power)
